...
export const Leer = async (req, res) => {
    ...
    try {
        const [rows] = await connectToDB.query("CALL Iniciar_Sesion_Leer(?)", [Cedula]);
        ...
        if (rows[0].length > 0) {
            ...
            const [Clave, Telefono] = await Promise.all([
                ...
                Desencriptar(Usuario.Telefono, Usuario.IV), // Segunda llamada
            ]);
    ...
    try {
       ...
       // Ejecutar ambas solicitudes en paralelo y esperar a que ambas terminen
        ...
        if(Clave.Estado && Telefono.Estado){
            ...
            const [result] = await connectToDB.query("CALL Iniciar_Sesion_Actualizar(?, ?, ?, ?, ?)", [Parametros.Cedula, Parametros.Nombre, Telefono.Contenido.encriptado, Clave.Contenido.encriptado, Clave.Contenido.iv]);
...
export const Eliminar = async (req, res) =>{
    ...
    try {
        const [result] = await connectToDB.query("CALL Iniciar_Sesion_Eliminar(?)", [Cedula]);
        if (result.affectedRows > 0) {
